...
const EditAssetModal = ({
  ...
  // Pre-fill form when selectedAsset changes
  useEffect(() => {
    if (selectedAsset) {
      reset({
        ...
        asset_brand: selectedAsset.asset_brand,
        asset_model: selectedAsset.asset_model,
        ...
        asset_description: selectedAsset.asset_description || "",
        asset_notes: selectedAsset.asset_notes || "",
      });
    }
  }, [selectedAsset, reset]);
  ...
  const onSubmit = async (data) => {
    if (!selectedAsset?.asset_id) {
      setFormError("No asset selected for update.");
      return;
    }
    ...
    try {
      ...
      const response = await axiosPublic.put(`/Assets/${selectedAsset?.asset_id}`, payload);
      ...
      if (response.status === 200) {
        ...
        setFormError(response.data?.message || "Failed to update asset.");
      }
    } catch (error) {
