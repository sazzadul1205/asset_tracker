...
const EditAssetCategoryModal = ({
  ...
  // Default Icon
  const placeholderIcon = selectedCategory?.iconImage
  ...
  // Prefill data when a category is selected
  useEffect(() => {
    if (selectedCategory) {
      reset({
        ...
        warranty: selectedCategory.warranty || "",
      });

      setSelectedColor(selectedCategory.selectedColor || "#ffffff");
      setIconImage(selectedCategory.iconImage || null);
    }
  ...
  const handleClose = () => {
    ...
    document.getElementById("Edit_Asset_Category_Modal")?.close();
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (iconImage && iconImage !== selectedCategory.iconImage) {
      ...
      const response = await axiosPublic.put(`/AssetCategory/${selectedCategory._id}`, payload);
      ...
      if (response.status === 200) {
        ...
        setFormError(response.data?.error || "Failed to update category");
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-bold text-gray-800">Edit Asset Category</h3>
        <button
          type="button"
          onClick={handleClose}
